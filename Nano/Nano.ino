...
void readDevKitCData() {
  if (Serial1.available() > 0) {
    ...
    Serial.printf("pH: %.2f | Temp: %.2f | RPM: %d\n",
                (float)doc["ph"],
...
void sendReadingsToDevKitC() {
  if (!isnan(temperature_mean) && !isnan(RPM_mean)) {
    Serial1.printf("{\"ph\":%.2f,\"temperature\":%.1f,\"rpm\":%.1f}\n", 0.0, temperature_mean, RPM_mean);
    Serial.printf("sent to devkitc {\"ph\":%.2f,\"temperature\":%.1f,\"rpm\":%.1f}\n", 0.0, temperature_mean, RPM_mean);
    temperature_mean = NAN;
    RPM_mean = NAN;
  }
}
...
void AcidOnAction() {digitalWrite(AcidPin,1);}
void AcidOffAction() {digitalWrite(AcidPin,0);}
void BaseOnAction() {digitalWrite(BasePin,1);}
void BaseOffAction() {digitalWrite(BasePin,0);}